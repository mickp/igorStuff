...
class App:  
    def __init__(self, master):
        ...
        o_func = OptionMenu(frame, self.func, "dummy")
        o_func.config(width=4)

        #Gain menu
        l_gain = Label(frame, text="Gain")
        o_gain = OptionMenu(frame, self.gain, "dummy")
    ...
    def acquire(self):
        ...
        lj.configPWM(PWMVAR * 16 * self.count)
        self.count += 1
...
class CaptureDevice():
    ...
    def configPWM(self, Value):
        self.d.getFeedback(u6.Timer0Config(TimerMode=0, Value=Value))
